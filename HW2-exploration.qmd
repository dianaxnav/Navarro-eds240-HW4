...
clean_bananas <- bananas %>% 
  clean_names() %>% 
  select(entity, bananas_index_kg, bananas_index_1000_kcalories, bananas_index_100g_protein, emissions_kg, land_use_kg)
```
...
banana_3 <- plot_ly(clean_bananas, y = ~entity, 
                    x = ~bananas_index_kg,
                    type = 'scatter',
                    mode = 'markers', 
               marker = list(size = 8)) 
